...
export function buildCodingAgentSessionDrill({
  ...
  return {
    ...
    summary: {
      ...
      sandboxContracts: new Set(items.map((item) => item.sandboxContract.evidenceArtifactPrefix)).size,
      humanApprovalRequired: items.filter((item) => item.sandboxContract.requiresHumanApproval).length,
...
function sessionToDrillItem(
  ...
  return {
    ...
    sandboxContract: session.sandboxContract,
...
function itemMarkdown(item: CodingAgentSessionDrillItem, index: number) {
  return [
    ...
    `- Sandbox boundary: ${item.sandboxContract.boundary}`,
    `- Receipt schema: ${item.sandboxContract.receiptSchema}`,
    `- Evidence prefix: ${item.sandboxContract.evidenceArtifactPrefix}`,
    `- Human approval required: ${item.sandboxContract.requiresHumanApproval ? "yes" : "no"}`,
    `- Reason: ${item.reason}`,
    ...
    "### Allowed Actions",
    "",
    ...item.sandboxContract.allowedActions.map((action) => `- ${action}`),
    "",
    "### Prohibited Actions",
    "",
    ...item.sandboxContract.prohibitedActions.map((action) => `- ${action}`),
    "",
    "### Required Commands",
    "",
