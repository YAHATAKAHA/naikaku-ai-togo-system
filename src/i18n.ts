...
const copies: Record<SupportedLocale, AppCopy> = {
  ja: {
    ...
    codingBriefs: {
      ...
      statusReceiptReviewApplied: (message, artifactDecision, verifiedPaths, unresolvedPaths, applied, skipped, reusedTranscriptRefs, transcriptContentMismatches) =>
        `${message} アーティファクト監査 ${artifactDecision}: ${verifiedPaths}件確認、${unresolvedPaths}件未解決${reusedTranscriptRefs ? `、transcript再利用 ${reusedTranscriptRefs}件` : ""}${transcriptContentMismatches ? `、内容不一致 ${transcriptContentMismatches}件` : ""}。Development Board へ ${applied}件反映、${skipped}件保留。`,
  ...
  en: {
    ...
    codingBriefs: {
      ...
      statusReceiptReviewApplied: (message, artifactDecision, verifiedPaths, unresolvedPaths, applied, skipped, reusedTranscriptRefs, transcriptContentMismatches) =>
        `${message} Artifact audit ${artifactDecision}: ${verifiedPaths} verified, ${unresolvedPaths} unresolved${reusedTranscriptRefs ? `, ${reusedTranscriptRefs} reused transcript refs` : ""}${transcriptContentMismatches ? `, ${transcriptContentMismatches} content mismatches` : ""}. Development Board applied ${applied}, held ${skipped}.`,
  ...
  "zh-Hans": {
    ...
    codingBriefs: {
      ...
      statusReceiptReviewApplied: (message, artifactDecision, verifiedPaths, unresolvedPaths, applied, skipped, reusedTranscriptRefs, transcriptContentMismatches) =>
        `${message} 工件审计 ${artifactDecision}：${verifiedPaths} 项已确认，${unresolvedPaths} 项未解决${reusedTranscriptRefs ? `，${reusedTranscriptRefs} 项 transcript 复用` : ""}${transcriptContentMismatches ? `，${transcriptContentMismatches} 项内容不匹配` : ""}。Development Board 已应用 ${applied} 项，保留 ${skipped} 项。`,
  ...
  "zh-Hant": {
    ...
    codingBriefs: {
      ...
      statusReceiptReviewApplied: (message, artifactDecision, verifiedPaths, unresolvedPaths, applied, skipped, reusedTranscriptRefs, transcriptContentMismatches) =>
        `${message} 工件稽核 ${artifactDecision}：${verifiedPaths} 項已確認，${unresolvedPaths} 項未解決${reusedTranscriptRefs ? `，${reusedTranscriptRefs} 項 transcript 重複使用` : ""}${transcriptContentMismatches ? `，${transcriptContentMismatches} 項內容不符` : ""}。Development Board 已套用 ${applied} 項，保留 ${skipped} 項。`,
  ...
  ko: {
    ...
    codingBriefs: {
      ...
      statusReceiptReviewApplied: (message, artifactDecision, verifiedPaths, unresolvedPaths, applied, skipped, reusedTranscriptRefs, transcriptContentMismatches) =>
        `${message} 아티팩트 감사 ${artifactDecision}: ${verifiedPaths}개 확인, ${unresolvedPaths}개 미해결${reusedTranscriptRefs ? `, transcript 재사용 ${reusedTranscriptRefs}개` : ""}${transcriptContentMismatches ? `, 내용 불일치 ${transcriptContentMismatches}개` : ""}. Development Board에 ${applied}개 적용, ${skipped}개 보류.`,
