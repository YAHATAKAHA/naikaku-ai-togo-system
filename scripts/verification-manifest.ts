...
async function loadCodingAgentRunnerIntake(reportPath: string): Promise<CodingAgentRunnerIntakeAuditDrillSummary> {
  const parsed = JSON.parse(await readFile(reportPath, "utf8")) as CodingAgentRunnerIntakeAuditDrillSummary;
  if (parsed.schema !== "naikaku.coding-agent-runner-intake-audit-drill.v1") {
    throw new Error("Coding-agent runner intake audit must use schema naikaku.coding-agent-runner-intake-audit-drill.v1.");
  }
  return parsed;
}
...
async function loadSecurityRedTeamDrill(reportPath: string): Promise<SecurityRedTeamDrillSummary> {
  const parsed = JSON.parse(await readFile(reportPath, "utf8")) as SecurityRedTeamDrillSummary;
  if (parsed.schema !== "naikaku.security-red-team-drill.v1") {
    throw new Error("Security red-team drill must use schema naikaku.security-red-team-drill.v1.");
  }
  return parsed;
}
...
function parseArgs(args: string[]): VerificationManifestOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--coding-agent-runner-intake") {
      options.codingAgentRunnerIntakePath = requireValue(args, index, arg);
      index += 1;
      continue;
    }
    ...
    if (arg === "--security-red-team-drill") {
      options.securityRedTeamDrillPath = requireValue(args, index, arg);
      index += 1;
      continue;
    }
...
Options:
  ...
  --coding-agent-runner-intake <path>
