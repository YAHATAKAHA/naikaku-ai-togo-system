...
`npm run verification:manifest` reads the latest localization drill, executor contract drill, production boundary drill, coding-agent dispatch drill, coding-agent dispatch simulation drill, coding-agent receipt drill, and release verification report, then writes `naikaku.verification-manifest.v1` to `output/verification/verification-manifest-latest.json`. The manifest fails if any supported locale drops the coding-agent handoff contract, if dispatch packaging writes prompts for production-held sessions, if dispatch simulation fails to write one pending receipt draft file per ready session, if it creates receipt drafts or receipt draft files for held sessions, if any executor profile loses scoped dry-run evidence, if a blocked production action reaches execution evidence, if the production verifier does not reject dry-run evidence with code 4, if the valid coding-agent receipt does not apply all board items, if the mismatched or out-of-scope receipt updates any board item, if release verification fails, or if the dry-run versus production boundary is unclear.
