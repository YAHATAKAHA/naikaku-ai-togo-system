...
npm run verify:all # run tests, build, gateway smoke, dry-run verification, production negative gate, and diff check
...
`npm run verify:all` is the recommended local and CI gate. It runs unit/domain tests, the production build, the gateway runner smoke, `npm run release:verify`, confirms `npm run release:verify:production` returns code 4, and finishes with `git diff --check`. CI should reuse the same command so local and remote verification share one contract.
