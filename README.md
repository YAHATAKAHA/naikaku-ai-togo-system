...
npm run verification:manifest # aggregate localization, executor, dispatch, simulation, runner, receipt, and release evidence
...
`npm run verification:manifest` reads the latest localization drill, executor contract drill, production boundary drill, coding-agent dispatch drill, coding-agent dispatch simulation drill, coding-agent runner manifest drill, coding-agent receipt drill, and release verification report, then writes `naikaku.verification-manifest.v1` to `output/verification/verification-manifest-latest.json`. The manifest fails if any supported locale drops the coding-agent handoff contract, if dispatch packaging writes prompts for production-held sessions, if dispatch simulation fails to write one pending receipt draft file per ready session, if runner manifest loses a ready task or receipt draft path, if it creates runner tasks for held sessions, if any executor profile loses scoped dry-run evidence, if a blocked production action reaches execution evidence, if the production verifier does not reject dry-run evidence with code 4, if the valid coding-agent receipt does not apply all board items, if the mismatched or out-of-scope receipt updates any board item, if release verification fails, or if the dry-run versus production boundary is unclear.

`npm run release:verify` first runs the localization drill, executor contract drill, coding-agent dispatch drill, coding-agent dispatch simulation drill, coding-agent runner manifest drill, coding-agent receipt drill, and release rehearsal drill, then turns the latest drill rehearsal into `naikaku.release-verification.v1`, runs the production boundary drill, and finally writes the verification manifest. It fails if localization contracts drift, dispatch packaging assigns held sessions, dispatch simulation overclaims execution readiness, runner manifest queues held work, executor contracts overclaim, coding-agent evidence gates accept mismatched or out-of-scope sandbox evidence, the production boundary stops rejecting dry-run evidence, warnings, blockers, schema drift, or secret leakage are present, release verification cannot pass for dry-run scope, or the manifest cannot prove every local gate. The workbench panel and local gateway expose the same release verifier for operator review and downloadable JSON. `npm run release:verify:production` is intentionally stricter: it returns code 4 while the evidence claim is still `dry-run`, so a sandbox drill cannot be mistaken for a production handoff.
