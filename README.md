...
It starts on `http://127.0.0.1:8787` by default and exposes health, provider test, cabinet run, automation plan, automation runbook, executor handoff, executor dry-run, executor evidence, team package, role workspace scaffold, product readiness, product release bundle, release rehearsal, release verification, development issue draft, coding agent brief, coding agent brief review, coding agent session bundle, coding agent session drill, coding agent session receipt, coding agent implementation evidence, coding agent artifact audit, sandbox capability, and sandbox policy-check endpoints. The GitHub CLI issue script is generated locally from issue drafts and must be run only inside a repository where `gh` is already authenticated.
...
npm run verification:manifest # aggregate localization, executor, receipt, and release evidence
...
`npm run verification:manifest` reads the latest localization drill, executor contract drill, production boundary drill, coding-agent receipt drill, and release verification report, then writes `naikaku.verification-manifest.v1` to `output/verification/verification-manifest-latest.json`. The manifest fails if any supported locale drops the coding-agent handoff contract, if any executor profile loses scoped dry-run evidence, if a blocked production action reaches execution evidence, if the production verifier does not reject dry-run evidence with code 4, if the valid coding-agent receipt does not apply all board items, if the mismatched receipt updates any board item, if release verification fails, or if the dry-run versus production boundary is unclear.

`npm run release:verify` first runs the localization drill, executor contract drill, coding-agent receipt drill, and release rehearsal drill, then turns the latest drill rehearsal into `naikaku.release-verification.v1`, runs the production boundary drill, and finally writes the verification manifest. It fails if localization contracts drift, executor contracts overclaim, coding-agent evidence gates accept mismatched evidence, the production boundary stops rejecting dry-run evidence, warnings, blockers, schema drift, or secret leakage are present, release verification cannot pass for dry-run scope, or the manifest cannot prove every local gate. The workbench panel and local gateway expose the same release verifier for operator review and downloadable JSON. `npm run release:verify:production` is intentionally stricter: it returns code 4 while the evidence claim is still `dry-run`, so a sandbox drill cannot be mistaken for a production handoff.
