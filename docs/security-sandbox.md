...
Executor-facing gateway routes now support runner authentication. Legacy `NAIKAKU_RUNNER_TOKEN` keeps shared-token local compatibility, while `NAIKAKU_RUNNER_CREDENTIALS` is preferred for real runner development. Scoped credentials bind `runnerId` to `token` or `tokenSha256`, executor profile ids, and optional rotation fields. When scoped credentials are configured, `/v1/executor/handoff` returns only ready actions in the authenticated runner's profile scope; executor run, executor evidence, ledger evidence writes, and coding-agent sandbox-runner execution return `403` if the payload includes out-of-scope profiles. The same scope functions are used by the gateway, unit tests, and `npm run runner-auth:drill`, whose mixed handoff/evidence probe proves a shell runner receives only shell actions and shell evidence from a broader payload. If neither auth mode is configured, `/health` reports `development-open` so local development is explicit rather than silently pretending to be production-secure. Malformed scoped credential JSON fails closed.
...
The release verifier makes that boundary executable. The workbench panel, local gateway, and `npm run release:verify` accept clean localization, executor contract, sandbox capability, security red-team, runner auth, coding-agent dispatch, coding-agent dispatch simulation, coding-agent runner manifest, coding-agent runner invocation, coding-agent runner intake audit, coding-agent runner self-test, coding-agent runner lease, coding-agent sandbox runner, coding-agent engineering self-simulation, coding-agent receipt, and release dry-run drills as sandbox evidence, then run a production boundary drill and write a verification manifest that records all gates. `npm run release:verify:production` returns code 4 until authenticated production runner evidence is attached. `npm run verify:all` treats that code 4 as the expected negative gate after tests, build, gateway smoke, fixture engineering self-simulation, the `engineering:auto-work-smoke` external CLI receipt-import path, and dry-run verification pass. CI should run the same command. This gives CI and operators a hard stop between "the drills work" and "real computer-control backends are safe to release."
...
The Coding Agent Runner Lease Drill adds the execution-ownership boundary that a real runner fleet will need. It consumes runner self-test artifacts, grants one scoped runner an active lease, treats same-runner retries as idempotent, blocks a competing runner while the lease is active, reclaims after expiry, denies a wrong-profile runner, and proves production-held sessions receive no active leases. The drill is included in `npm run release:verify` and the verification manifest. The local gateway also exposes the same lease claim at `/v1/development/coding-briefs/runner-lease`, records issued active lease ids in memory for the current gateway process, and the sandbox-runner execution route returns `409` before command execution unless every ready preflight task has an active, unexpired lease owned by the authenticated runner and issued by that gateway. This remains pre-execution evidence: it does not run commands, inspect prompts, update the Development Board, push Git, deploy, or prove implementation.

The Coding Agent Sandbox Runner Drill is deliberately narrower than a real coding agent but stronger than a no-execution self-test. Before execution, the sandbox runner preflight checks bundle/session match, self-test readiness, allowlisted commands, expected process count, and safe artifact paths without running commands. The runner then consumes runner self-test artifacts, executes only allowlisted local verification commands, writes per-session transcripts and evidence artifacts under the session sandbox prefix, submits and reviews a receipt, and audits artifact existence plus transcript command/exit-code content. Its negative production-held case must execute zero commands. The Workbench can invoke the same preflight and runner through the local gateway, automatically claim a runner lease, and pass that lease into the execution route. The execution route reruns preflight server-side before executing, validates runner auth scope, lease ownership, and current-process gateway issuance, returns `409` without command execution when preflight or lease validation is not ready, and uses runner authentication when `NAIKAKU_RUNNER_TOKEN` or `NAIKAKU_RUNNER_CREDENTIALS` is configured. Scoped credentials must match every ready session executor profile before execution starts. `npm run coding-agent:gateway-smoke` starts a temporary local gateway with synthetic scoped credentials, proves missing and fabricated unissued leases are blocked over HTTP, executes the sandbox-runner route with a gateway-issued lease, proves the artifact-audit route can verify a temporary Git worktree change, and proves a clean tracked changed-file claim stays blocked as `needs-artifacts`. This proves the local runner plumbing can actually run safe commands and produce reviewable evidence, but it still does not implement backlog work, reconcile the Development Board, push Git, deploy, call providers, control browsers/desktops, or prove production readiness.

The Engineering Launchpad makes these boundaries visible before a Mac operator starts work. It describes the current minimum permissions as repository-scoped file read/write, `output/` transcript/evidence writes, allowlisted shell verification, and Git status/diff reads. Browser automation, desktop Accessibility/Screen Recording, MCP tools, Git push, deploy, and external-send are named as optional executor-profile capabilities that require explicit allowlists or human approval. The launchpad does not grant permission by itself; it only calls the existing cabinet, dispatch, preflight, sandbox-runner, and issue-export flows so OpenClaw-style capabilities remain governed rather than becoming blanket host control.

The Coding Agent Engineering Self-Simulation goes one step closer to real engineering while staying safely bounded. It creates a temporary fixture Git workspace under ignored output, commits a broken local fixture, confirms its test fails, patches one fixture source file, reruns the fixture test, and sends the resulting transcripts, diff artifact, changed-file path, receipt, implementation evidence, and artifact audit through the same anti-fake chain. It also proves two bad evidence paths stay blocked: failed verification output cannot become accepted implementation evidence, and a clean tracked fixture file cannot be claimed as changed work. Its changed-file audit is backed by the fixture Git status. The boundary is explicit: the fixture drill does not touch production code, call providers, browse, use MCP tools, deploy, push Git, reconcile real Development Board work, or prove a real model-run implementation.
...
1. Move scoped runner credentials from local environment JSON into a server-side secret manager with audited rotation.
