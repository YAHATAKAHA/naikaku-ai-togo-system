...
The Coding Agent Runner Invocation Drill narrows the handoff gap between a runner manifest and a future Codex/OpenClaw-style coding runner. It writes one JSON/Markdown invocation file for each ready runner task, carrying the prompt path, receipt draft path, pending commands, evidence targets, runner instructions, and stop conditions. Held and production-held tasks remain visible but receive zero executable invocation files. The invocation files still contain null exit codes and pending command contracts only; they are not implementation receipts, do not prove command execution, and cannot update the Development Board.
