...
The Coding Agent Receipt Drill turns that boundary into a repeatable local regression gate. It creates local sandbox artifacts for a valid receipt and a deliberately mismatched receipt, then fails if the valid path cannot verify or if the mismatched path updates any Development Board item. The drill is useful as anti-fake evidence for the product itself, but it remains local drill evidence: it does not prove a real external coding agent, production runner, browser, provider, deploy target, or Git remote performed the work.
