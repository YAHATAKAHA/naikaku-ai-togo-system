...
Coding Agent Dispatch Manifests are the outgoing package boundary. Ready sessions can receive prompt paths, expected transcript refs, evidence artifact paths, and a receipt template; held or production-evidence-held sessions remain listed but do not receive assignable prompt files. The dispatch drill writes a valid local package and a production-held negative package, then fails if held work becomes dispatchable. A dispatch package is useful handoff material for a governed coding agent, but it is still not proof that code was edited or tests were run.
