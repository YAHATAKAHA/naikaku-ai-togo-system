...
The Coding Agent Sandbox Runner Drill is deliberately narrower than a real coding agent but stronger than a no-execution self-test. It consumes runner self-test artifacts, executes only allowlisted local verification commands, writes per-session transcripts and evidence artifacts under the session sandbox prefix, submits and reviews a receipt, and audits artifact existence plus transcript command/exit-code content. Its negative production-held case must execute zero commands. The Workbench can invoke the same runner through the local gateway, and the route uses runner authentication when `NAIKAKU_RUNNER_TOKEN` is configured. This proves the local runner plumbing can actually run safe commands and produce reviewable evidence, but it still does not implement backlog work, reconcile the Development Board, push Git, deploy, call providers, control browsers/desktops, or prove production readiness.
