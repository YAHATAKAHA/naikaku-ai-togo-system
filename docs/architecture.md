...
`VerificationManifest` is the CLI-level aggregation gate for local self-simulation. It reads the localization drill summary, executor contract drill summary, production boundary drill summary, coding-agent dispatch drill summary, coding-agent dispatch simulation summary, coding-agent receipt drill summary, and release verification report, then writes `naikaku.verification-manifest.v1`. The manifest checks that supported locales preserve the coding-agent handoff contract, dispatch packaging writes ready prompts while keeping production-held sessions unassigned, dispatch simulation writes one pending receipt draft file for each ready session and zero draft files for production-held sessions, every executor profile preserves scoped dry-run evidence while blocked production actions stay held, production verification rejects dry-run evidence with exit code 4, the valid coding-agent receipt applied all Development Board items, the mismatched receipt applied none, out-of-scope receipt artifacts outside the session sandbox prefix applied none, release verification passed, and the dry-run versus production boundary remains explicit. It does not replace the underlying drill or verification reports; it points to them so operators and CI can review one summary without losing source evidence.
