...
`CodingAgentImplementationArtifactAudit` adds a local anti-fake gate before status is updated. It checks that accepted implementation evidence references safe relative changed-file paths and command transcript paths. When the gateway is available, those paths must be real files inside the current sandbox workspace, and the audit records local byte counts, modified times, and `sha256` fingerprints for later review. The summary separates total references from unique files so repeated transcript references cannot inflate the apparent evidence set, and empty command transcripts remain `needs-artifacts`. Browser-only fallback can flag path safety but leaves existence and fingerprints unresolved. The audit still does not rerun commands, prove command output contents, or prove that a remote runner produced those files.
