...
`VerifyAll` is the combined local and CI verification entrypoint. It runs unit/domain tests, production build, full dry-run release verification with localization, executor, sandbox capability, security red-team, runner auth, coding-agent dispatch, runner manifest, runner invocation, runner intake audit, runner self-test, sandbox-runner, receipt, release rehearsal, production boundary, and verification manifest gates, then confirms production verification still returns code 4 for dry-run evidence. This keeps the "good drill passes, production claim still blocked" invariant visible in one command.
