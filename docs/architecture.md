...
`CodingAgentDispatchManifest` turns a reviewed session bundle and optional session drill into `naikaku.coding-agent-dispatch-manifest.v1`. It is the practical package boundary for Codex-like implementation agents: ready sessions receive prompt paths under `prompts/`, a shared `receipt-template.json`, expected transcript references, expected evidence artifact paths, and the same sandbox contract; held or production-evidence-held sessions remain visible but do not receive assignable prompt paths. `CodingAgentDispatchArchive` then packages the manifest, prompt file contents, receipt template, and README into `naikaku.coding-agent-dispatch-archive.v1` for one-file UI download and CLI file writing. The workbench Coding Agent panel, local gateway, and CLI drill share this manifest path: the UI can prepare the package with gateway/local fallback and export JSON/Markdown, while the CLI writes a valid local dispatch package and a production-held negative package, then fails if held sessions become assignable. The manifest and archive still do not execute code, call providers, browse, deploy, or push Git.
