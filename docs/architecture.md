...
`CodingAgentSandboxRunnerDrill` is the first local executable runner bridge. It consumes `naikaku.coding-agent-runner-self-test.v1`, executes only allowlisted local verification commands (`npm run test` and `npm run build` at this stage), writes session-scoped command transcripts, changed-file summary placeholders, evidence artifacts, a submitted receipt, receipt review, implementation evidence, and artifact audit, then emits `naikaku.coding-agent-sandbox-runner-drill.v1`. It also runs a production-held negative case with zero command execution. The same runner core is exposed through the local gateway so the Workbench can start a sandbox runner after dispatch packaging and runner self-test are ready, then download the sandbox runner report, receipt review, and implementation evidence. This proves the local command/evidence/receipt/audit path is runnable, but it deliberately does not call a coding model, implement backlog work, reconcile the Development Board, push Git, deploy, or claim production evidence.
