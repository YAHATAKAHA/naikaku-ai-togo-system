...
- Editable Provider Readiness matrix with one row per role, provider/endpoint/model/API alias inputs, session-only configuration checks, explicitly non-ready local fallback results, audit events, and export.
...
Provider Readiness is the operator-facing preflight for those adapters. The frontend derives static readiness from each role's endpoint, model, alias, and session-only key state. `/v1/provider/test` checks configuration and key availability through the local gateway without calling a billable model endpoint. A session-only key is never persisted and does not replace the gateway environment variable needed by `live` mode. If the gateway is offline, the local fallback validates endpoint/model shape but stays `unchecked`; it cannot make a provider look ready. Results are stored locally as status metadata only and can be exported as `naikaku.provider-readiness.v1`.
...
Executor-facing gateway routes have a runner authentication gate. In local development, missing runner credentials is reported as `development-open` in `/health`. Legacy `NAIKAKU_RUNNER_TOKEN` keeps shared-token compatibility for local teams, while `NAIKAKU_RUNNER_CREDENTIALS` is preferred before real runner services attach. Scoped credentials bind `runnerId` to `token` or `tokenSha256`, executor profiles, and optional rotation fields such as `notBefore`, `rotatedAt`, and `expiresAt`. Scoped mode takes precedence over the shared token, fails closed when malformed, filters `/v1/executor/handoff` to the authenticated runner profile, and returns `403` when executor run, evidence, ledger evidence writes, or coding-agent sandbox-runner execution include out-of-scope profiles.
...
`VerificationManifest` is the CLI-level aggregation gate for local self-simulation. It reads the localization drill summary, executor contract drill summary, sandbox capability drill summary, security red-team drill summary, runner auth drill summary, production boundary drill summary, coding-agent dispatch drill summary, coding-agent dispatch simulation summary, coding-agent runner manifest summary, coding-agent runner invocation summary, coding-agent runner intake audit summary, coding-agent runner self-test summary, coding-agent runner lease summary, coding-agent sandbox runner summary, coding-agent engineering self-simulation summary, coding-agent receipt drill summary, and release verification report, then writes `naikaku.verification-manifest.v1`. The manifest checks that supported locales preserve the coding-agent handoff contract, dispatch packaging writes ready prompts while keeping production-held sessions unassigned, dispatch simulation writes one pending receipt draft file for each ready session and zero draft files for production-held sessions, runner manifest conversion turns each ready pending draft into one runner task while keeping production-held sessions out of runner queues, runner invocation packaging writes one executable handoff file per ready task and zero files for production-held tasks, runner intake accepts only readable ready invocation files with pending commands, scoped evidence paths, and zero blocked security classifications, runner self-test keeps commands non-executed, runner lease preserves exclusive ownership, idempotent same-runner retries, duplicate-runner blocking, expiry reclaim, profile-scope denial, and production-held non-assignment, sandbox runner executes only allowlisted local verification commands and produces audited drill receipts while keeping production-held tasks unrun, engineering self-simulation proves fail-before/pass-after fixture patching with verified receipt, artifact fingerprints, transcript checks, fixture Git worktree status, failed-test rejection, and clean-worktree claim rejection, every executor profile preserves scoped dry-run evidence while blocked production actions stay held, the sandbox capability registry preserves readiness checks, approval gates, blocked reasons, evidence artifacts, role coverage, and kill-switch blocking, hostile prompt, credential, localhost/control-plane, metadata, Git, deploy, and external-send cases stay blocked by the security red-team drill, runner authentication preserves scoped credentials, token-hash acceptance, profile limits, handoff/evidence filtering, expiry rejection, and fail-closed malformed config, production verification rejects dry-run evidence with exit code 4, the valid coding-agent receipt applied all Development Board items, the mismatched receipt applied none, out-of-scope receipt artifacts outside the session sandbox prefix applied none, release verification passed, and the dry-run versus production boundary remains explicit. It does not replace the underlying drill or verification reports; it points to them so operators and CI can review one summary without losing source evidence.

`LocalizationDrill` is the language-contract self-simulation for the coding-agent path. It runs the default Development Board through coding-agent briefs, brief review, session bundle, dispatch manifest, dispatch simulation, runner manifest, runner invocation package, runner intake audit, assignment drill, and receipt template for Japanese, English, Simplified Chinese, Traditional Chinese, and Korean. The drill fails if Japanese is not first, if operator-language instructions are missing, if commands, paths, JSON schema keys, allowed actions, prohibited actions, session sandbox contracts, dispatch prompt paths, runner task paths, runner invocation file paths, runner intake decisions, or evidence requirements stop being stable machine contracts, or if receipt templates claim implementation without evidence.
...
`SandboxCapabilityDrill` is the readiness-contract self-simulation for computer-use profiles. It writes `naikaku.sandbox-capability-drill.v1` from the active default roles, executor profiles, and sandbox policy, then builds a kill-switch-open negative registry. The drill fails if any profile loses its five runner readiness checks, required approval visibility, blocked reasons, supported evidence artifacts, role coverage, or if opening the kill switch no longer blocks every runner profile. It does not execute browser, desktop, shell, MCP, provider, deployment, or Git actions; it proves that the policy contract is machine-checkable before real runners attach.

`SecurityRedTeamDrill` is the hostile-input self-simulation before runner handoff. It classifies deterministic prompt-injection, credential, localhost/control-plane, metadata, Git mutation, production deploy, external-send, high-impact shell, and safe allowlisted URL fixtures through the local security classifier plus `SandboxPolicy`. The drill fails if hostile cases are not blocked, if bounded shell verification stops requiring approval, if the safe allowlisted research case is blocked, or if any fixture claims execution. It does not call a model, browse, run shell commands, call MCP tools, deploy, or touch Git.
...
`VerifyAll` is the combined local and CI verification entrypoint. It runs unit/domain tests, production build, a local gateway runner smoke with scoped credentials and issued-lease sandbox execution, the fixture engineering self-simulation, full dry-run release verification with localization, executor, sandbox capability, security red-team, runner auth, coding-agent dispatch, runner manifest, runner invocation, runner intake audit, runner self-test, runner lease, sandbox-runner, receipt, release rehearsal, production boundary, and verification manifest gates, then confirms production verification still returns code 4 for dry-run evidence. This keeps the "good drill passes, production claim still blocked" invariant visible in one command.
...
`CodingAgentDispatchManifest` turns a reviewed session bundle and optional session drill into `naikaku.coding-agent-dispatch-manifest.v1`. It is the practical package boundary for Codex-like implementation agents: ready sessions receive prompt paths under `prompts/`, a shared `receipt-template.json`, expected transcript references, expected evidence artifact paths, and the same sandbox contract; held or production-evidence-held sessions remain visible but do not receive assignable prompt paths. `CodingAgentDispatchArchive` then packages the manifest, prompt file contents, receipt template, and README into `naikaku.coding-agent-dispatch-archive.v1` for one-file UI download and CLI file writing. `CodingAgentDispatchArchiveAudit` validates that the archive schema, file inventory, manifest, prompt files, held-session gating, receipt template, and path safety are internally consistent before a package is handed off. The workbench Coding Agent panel, local gateway, and CLI drill share this manifest path: the UI can prepare the package with gateway/local fallback and export JSON/Markdown, while the CLI writes a valid local dispatch package and a production-held negative package, then fails if held sessions become assignable or archive audit fails. The manifest, archive, and audit still do not execute code, call providers, browse, deploy, or push Git.

`EngineeringLaunchpad` is the operator-facing entrypoint for that chain. It sits near the top of the main workspace so a Mac operator can see where to enter the mission, run the supervising cabinet, prepare the coding-agent package, preflight local permissions, run the sandbox runner, and export issue scripts without hunting through every internal artifact panel. It does not introduce a second execution path; its buttons call the same cabinet, dispatch, preflight, sandbox-runner, and issue export functions used by the detailed panels. The permission copy is deliberately Mac-first: current code work needs repository read/write, `output/` evidence writes, allowlisted shell verification, and Git status/diff reads; browser automation, desktop Accessibility/Screen Recording, MCP tools, Git push, deploy, and external send stay profile-scoped and approval-gated.

`CodingAgentRunnerManifest` converts a verified dispatch simulation plus pending receipt draft files into `naikaku.coding-agent-runner-manifest.v1`. It is the runner-facing bridge for future Codex/OpenClaw-style executors: each ready task carries its prompt path, receipt draft path, executor profile, runner id, pending commands, expected transcripts, evidence artifact targets, and stop conditions. The CLI drill reads the actual `valid/receipt-drafts/*.json` files written by the simulation drill and fails if any ready session loses its receipt draft path, if a command claims an exit code before real execution, if unsafe paths appear, or if production-held sessions become runner tasks. It does not execute the runner; completion still requires a filled receipt, artifact audit, and Development Board reconciliation.

`CodingAgentRunnerInvocationPackage` turns a runner manifest into `naikaku.coding-agent-runner-invocation-package.v1` plus one `naikaku.coding-agent-runner-invocation.v1` JSON/Markdown file per ready runner task. Each invocation file preserves the prompt path, receipt draft path, receipt template path, pending command contracts, transcript targets, expected evidence artifacts, runner instructions, stop conditions, operator locale, executor profile, and runner id. Held and production-held tasks stay visible in the package summary but receive zero executable invocation files. The CLI drill writes those files locally, the gateway exposes the same package builder, and the Workbench downloads JSON/Markdown invocation packages after dispatch preparation. This makes the handoff more directly consumable by future Codex/OpenClaw-style coding runners while keeping all command exit codes null and all implementation claims pending until receipt review and artifact audit.

`ExternalRunnerHandoff` is the adapter-first bridge to existing open-source and locally authenticated coding runners. It consumes the engineering simulation's adapter registry and runner invocation package, then writes `naikaku.external-runner-handoff.v1`, per-session task Markdown, and `naikaku.external-runner-adapter-job.v1` command jobs for a selected user-installed adapter such as OpenHands, Codex CLI, Claude Code, or Qwen Code. The default OpenHands path is review-only until upstream license review, installation, and operator approval are recorded. Fixed Workbench templates also support the locally authenticated coding CLIs, but they remain scoped commands: Qwen Code runs with controlled Auto mode rather than YOLO, a bounded session/tool budget, JSON output, and a required Naikaku receipt. After the gates pass, `engineering:run-adapter` can launch the user-installed CLI from the job JSON, capture stdout/stderr transcripts, write a `naikaku.external-runner-adapter-execution-receipt.v1` for each job, and return a `naikaku.external-runner-adapter-run.v1` summary. It checks whether the external runner wrote the expected Naikaku session receipt after the command started, so stale receipts do not unlock implementation review. Repository coding invocations are accepted only by compatible coding/shell adapters; desktop-control adapters such as OpenClaw or Hammerspoon must use Mac runner contracts instead of consuming coding tasks directly. The bridge never installs upstream software or grants permissions by itself; it exists so Naikaku can act as a governed add-on around mature open-source execution layers while keeping cabinet governance, evidence, receipts, and permission gates in this repository.

`EngineeringAdapterSelfTest` is the no-provider proof for that bridge. It creates a generated fixture Git workspace, writes a Naikaku adapter job, launches a deterministic fake external CLI runner through `engineering:run-adapter`, observes a failing fixture test, patches one fixture source file, reruns the test, writes the expected Naikaku session receipt, and sends that receipt through implementation evidence and artifact audit. This proves the adapter bridge can do real command-line work and return reviewable evidence without requiring OpenHands or model credentials during open-source smoke testing. It remains fixture-scoped and does not claim real backlog completion.

`CodingAgentRunnerIntakeAudit` reviews a runner invocation package before a coding runner consumes it. The CLI drill reads the invocation package and confirms the expected JSON/Markdown invocation files are readable, each accepted item has safe prompt, receipt, transcript, and evidence paths, commands remain pending with null exit codes, receipt-return instructions and stop conditions are present, and production-held tasks receive zero accepted intakes. The gateway exposes the same intake builder, and the Workbench prepares JSON/Markdown intake downloads between runner invocation packaging and runner self-test. This is a pre-runner acceptance gate, not execution evidence; it deliberately does not read prompt contents, run commands, inspect changed files, call providers, push Git, or claim completion.

`CodingAgentRunnerLease` is the execution-ownership gate between runner self-test and sandbox execution. It turns `naikaku.coding-agent-runner-self-test.v1` into `naikaku.coding-agent-runner-lease.v1`, where would-run tasks start available, held/blocked self-test items cannot be leased, and each active lease binds one session to one runner id, executor profile, issue time, and expiry time. Claiming is idempotent for the owning runner, blocked for competing runners while the lease is active, reclaimable after expiry, and denied when the runner's executor profile scope does not match the task. The CLI drill writes valid and production-held lease ledgers plus a `naikaku.coding-agent-runner-lease-drill.v1` summary, and the verification manifest fails if duplicate ownership, missing expiry reclaim, wrong-profile access, or production-held leasing appears. The local gateway exposes the same claim path at `/v1/development/coding-briefs/runner-lease`; the Workbench calls it automatically before sandbox-runner execution, and `/v1/development/coding-briefs/sandbox-runner` validates that every ready preflight session has an active, unexpired lease owned by the authenticated runner and issued by the current gateway process. This is still local queue ownership evidence only, not a distributed scheduler; it does not execute commands, inspect prompts, or prove implementation.

`CodingAgentSandboxRunnerPreflight` checks the runner self-test and session bundle before local execution. It verifies bundle/session match, self-test readiness, allowlisted commands, expected process count, evidence paths, and the honesty boundary without running commands. `CodingAgentSandboxRunnerDrill` is the first local executable runner bridge. It consumes `naikaku.coding-agent-runner-self-test.v1`, executes only allowlisted local verification commands (`npm run test` and `npm run build` at this stage), writes session-scoped command transcripts, changed-file summary placeholders, evidence artifacts, a submitted receipt, receipt review, implementation evidence, and artifact audit, then emits `naikaku.coding-agent-sandbox-runner-drill.v1`. It also runs a production-held negative case with zero command execution. The same preflight and runner core are exposed through the local gateway so the Workbench can check readiness, claim leases, start a sandbox runner after dispatch packaging and runner self-test are ready, then download the preflight, sandbox runner report, receipt review, and implementation evidence. The gateway execution route reruns the preflight server-side, validates the runner lease ledger and current gateway issuance record, returns it with successful execution results, and returns `409` without command execution when the preflight or lease validation is not ready. `CodingAgentGatewayRunnerSmoke` starts the gateway with synthetic scoped runner credentials, proves the HTTP route rejects missing and unissued leases before accepting a gateway-issued lease and executing the same sandbox runner, verifies the artifact-audit route can see a real temporary Git worktree change, and proves a clean tracked changed-file claim stays `needs-artifacts`. This proves the local command/evidence/receipt/audit path is runnable, but it deliberately does not call a coding model, implement backlog work, reconcile the Development Board, push Git, deploy, or claim production evidence.

`CodingAgentEngineeringSelfSimulation` narrows the remaining gap between "runner can execute commands" and "runner can close a coding loop." The CLI creates a temporary fixture Git workspace under `output/coding-agent-engineering-self-simulation`, commits a broken fixture implementation, observes the fixture test fail, patches one fixture source file, reruns the test, writes transcripts and a patch artifact under the session evidence prefix, submits a receipt, builds implementation evidence, and audits the artifacts plus fixture Git worktree status. It also runs two negative evidence probes: a failed-test receipt must stay blocked, and a clean tracked file claimed as changed must stay `needs-artifacts`. It is intentionally fixture-scoped: it proves the local edit-test-receipt-audit mechanics can close for a tiny task and reject common fake-completion evidence, not that a model implemented a real Development Board item or that production runner evidence exists.
...
`CodingAgentImplementationArtifactAudit` adds a local anti-fake gate before status is updated. It checks that accepted implementation evidence references safe relative changed-file paths, evidence artifact paths, and command transcript paths. When the gateway is available, those paths must be real files inside the current sandbox workspace, and the audit records local byte counts, modified times, and `sha256` fingerprints for later review. Gateway-backed audits also check changed-file references against `git status --porcelain`, so an existing clean file cannot be claimed as implementation work without a worktree change. The summary separates total references from unique files so repeated references cannot inflate the apparent evidence set. Evidence entries that only say "attached" without a local artifact path, evidence artifacts reused across required evidence items, changed files reused across multiple sessions, clean changed-file paths, empty command transcripts, transcript files reused by multiple command results, and transcripts that do not mention the expected command plus exit code remain `needs-artifacts` for automatic Development Board reconciliation. Browser-only fallback can flag path safety but leaves existence, fingerprints, transcript content, and worktree status checks unresolved. The audit still does not rerun commands, prove command output truthfulness, or prove that a remote runner produced those files.
