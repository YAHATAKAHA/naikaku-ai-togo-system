...
`VerificationManifest` is the CLI-level aggregation gate for local self-simulation. It reads the localization drill summary, executor contract drill summary, production boundary drill summary, coding-agent receipt drill summary, and release verification report, then writes `naikaku.verification-manifest.v1`. The manifest checks that supported locales preserve the coding-agent handoff contract, every executor profile preserves scoped dry-run evidence while blocked production actions stay held, production verification rejects dry-run evidence with exit code 4, the valid coding-agent receipt applied all Development Board items, the mismatched receipt applied none, release verification passed, and the dry-run versus production boundary remains explicit. It does not replace the underlying drill or verification reports; it points to them so operators and CI can review one summary without losing source evidence.

`LocalizationDrill` is the language-contract self-simulation for the coding-agent path. It runs the default Development Board through coding-agent briefs, brief review, session bundle, assignment drill, and receipt template for Japanese, English, Simplified Chinese, Traditional Chinese, and Korean. The drill fails if Japanese is not first, if operator-language instructions are missing, if commands, paths, JSON schema keys, allowed actions, prohibited actions, session sandbox contracts, or evidence requirements stop being stable machine contracts, or if receipt templates claim implementation without evidence.
