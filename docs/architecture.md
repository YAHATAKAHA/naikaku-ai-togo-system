...
`ReleaseVerificationReport` is the machine gate after rehearsal. It validates a rehearsal report into `naikaku.release-verification.v1`, checking schema, clear rehearsal gates, secret redaction, explicit evidence claims, and optional production evidence. Dry-run verification can pass for sandbox drill scope. Production verification fails with `not-production-ready` until the evidence claim is upgraded from dry-run to production, which prevents a 100/100 drill from being treated as a real external handoff.
